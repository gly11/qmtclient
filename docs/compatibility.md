...
| 规划中 | `/v1` | 适配 qmtserver `0.5.0` 实时行情订阅 endpoints 和 WebSocket quote events。 |
...
- 规划中：`POST /v1/market/subscriptions`
- 规划中：`GET /v1/market/subscriptions`
- 规划中：`GET /v1/market/subscriptions/{subscription_id}`
- 规划中：`DELETE /v1/market/subscriptions/{subscription_id}`
...
规划中的实时行情订阅使用 qmtserver `schema="market.subscription.v1"` 和
`schema="market.quote.v1"`。qmtclient 将通过 `client.market` 封装订阅生命周期，并继续通过
