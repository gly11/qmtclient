...
| `0.5.0` | `/v1` | 适配 qmtserver `0.5.0` 实时行情订阅 endpoints 和 WebSocket quote events。 |
...
- `qmtclient market-subscribe-check`
...
实时行情订阅使用 qmtserver `schema="market.subscription.v1"` 和
