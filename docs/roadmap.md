...
|-- market.py       # market subscription API helpers
|-- models.py       # typed response schema helpers
|-- proxy.py        # 动态 RPC proxy
|-- trader.py       # trader readonly API helpers
...
- 诊断优先 CLI，用于连接检查、RPC methods 查看、WebSocket 冒烟检查、只读 market 冒烟检查和本地文件校验。
- `client.account` 只读查询优先调用 qmtserver `0.4.0` 稳定 `/v1/trader/*` endpoints。

下一版本已推进：

- `client.market` 封装 qmtserver `0.5.0` 实时行情订阅 lifecycle endpoints。
- `FakeQmtClient` 支持订阅状态、fixture 和 `market_quote` 事件回放。
- CLI 增加 `market-subscribe-check` 只读订阅冒烟检查。
...
- 诊断优先 CLI：提供连接检查、兼容性检查、RPC methods 查看、WebSocket 冒烟检查、只读 market 冒烟检查、snapshot/fixture 校验。
- 体验增强：支持有限 retry/backoff、WebSocket reconnect、显式启用的 `MemoryCache`、Pandas/Arrow 延迟导入转换工具。

## 后续方向

- qmtserver `0.5.0` 正式发布后，补充真实 qmtserver 手动冒烟记录和版本兼容说明。
- 跟随 qmtserver 批量/snapshot 契约演进，优先更新兼容性文档和契约测试，再调整客户端实现。
- 跟随 qmtserver trader readonly 契约演进，保持 `client.account` 的只读查询稳定。
- 当 qmtserver 新增 `/v2` 或新 schema version 时，先补兼容性测试和迁移说明，再扩展 facade。
- 持续完善真实 qmtserver 手动冒烟指南；CLI 不提供真实交易快捷入口。
- 持续保持基础安装轻量；研究体验能力优先通过延迟导入或 optional extra 提供。

## 验证分层

qmtclient 的默认开发验收不要求连接真实 qmtserver、MiniQMT 或 `xtquant`。真实 qmtserver 验证作为可选集成冒烟检查，用于补充确认部署环境和协议兼容性，不进入默认单元测试和本地质量门禁。
